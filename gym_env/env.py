...
class HoldemTable(Env):
    ...
    def __init__(
        ...
        self.observation_space = Box(
            low=-100.0, high=100.0, shape=(1000,), dtype=np.float32
        )
    ...
    def reset(self, seed=None, options=None):
        ...
        # Return observation in correct format
        if self.array_everything is not None:
            obs = self.array_everything.astype(np.float32)
            # Ensure observation matches observation space bounds
    ...
    def step(self, action):  # pylint: disable=arguments-differ
        ...
        # and will get a call from to the step function externally
        self.reward = 0
        ...
        else:  # action received from player shell (e.g. not autoplay)
        ...
        # Ensure array_everything is properly formatted
        if self.array_everything is not None:
            obs = self.array_everything.astype(np.float32)
            # Ensure observation matches observation space bounds
            obs = np.clip(obs, self.observation_space.low, self.observation_space.high)
    ...
    def _get_environment(self):
        ...
        arr1 = np.array(
            list(flatten(self.player_data.__dict__.values())), dtype=np.float32
        )
        arr2 = np.array(
            list(flatten(self.community_data.__dict__.values())), dtype=np.float32
        )
        arr3 = np.array(
        ...
        self.array_everything = (
            np.concatenate([arr1, arr2, arr3]).flatten().astype(np.float32)
        )
        ...
        if hasattr(self, "observation_space") and self.array_everything is not None:
            ...
            if self.observation_space.shape != expected_shape:
                # Update observation space with correct shape and bounds
                self.observation_space = Box(
                    low=-100.0, high=100.0, shape=expected_shape, dtype=np.float32
                )
    ...
    def _calculate_reward(self, last_action):
        ...
        if self.done:
            ...
            log.debug(f"agent has reward {self.reward}")
